...
ShaderEffect.prototype.render = function(context, target) {
  ...
  self.setDefaultUniforms(context, self.uniforms)

  // Render our effect
  self.renderer.render(self.scene, self.camera, target)
}

ShaderEffect.prototype.setDefaultUniforms = function(context, uniforms) {
  // FIXME: SHOULD MODIFY THREE.js to get missing uniforms during compilation
  //        from camera !!!!
  uniforms.lucy_time.value   = context.time
  uniforms.lucy_song.value   = context.song
  uniforms.lucy_aspect.value = context.aspect || this.renderer.camera.aspect
}
