...
ShaderEffect.prototype.setup = function(options, sub) {
  ...
  // lazy loading
  self.renderer = require('./webgl_renderer')

  console.assert(sub, 'ShaderEffect needs a sub scene to work')
...
ShaderEffect.prototype.render = function(time, target) {
  ...
  this.sub.render(time, self.target)

  // Copy target into our tDiffuse texture
  this.uniforms.tDiffuse.value = self.target
